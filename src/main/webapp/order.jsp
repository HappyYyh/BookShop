...
<div class="container">
    <p>
        <h2 style="font-family: 华文楷体">请您核对信息:</h2>
    </p>
    <table class="table table-bordered table-striped table-hover" style="margin-top: 50px">
        ...
        <c:forEach items="${OrderInfo.forOrder }" var="o" varStatus = "status">
            <tr>
                <c:if test="${status.count==1}">
                    <td style="display:table-cell;vertical-align:middle;font-weight: bold;" align="center"
                        rowspan="${OrderInfo.forOrder.size()}">${OrderInfo.orderId}</td>
                </c:if>

                <td align="center">${o.book.name}</td>
                <td align="center"><img src="/imgs/${o.book.photo}" style="width: 50px;height: 50px"></td>
                <td align="center">${o.num}</td>
                <td align="center">${o.book.nowPrice}</td>
                <c:if test="${status.count==1}">
                    <td style="display:table-cell;vertical-align:middle;font-weight: bold;" align="center"
                        rowspan="${OrderInfo.forOrder.size()}">${o.user.realname}</td>
                </c:if>
                <c:if test="${status.count==1}">
                    <td style="display:table-cell;vertical-align:middle;font-weight: bold;" align="center"
                        rowspan="${OrderInfo.forOrder.size()}">${o.user.address}</td>
                </c:if>
                <c:if test="${status.count==1}">
                    <td style="display:table-cell;vertical-align:middle;font-weight: bold;" align="center"
                        rowspan="${OrderInfo.forOrder.size()}">${o.user.phone}</td>
                </c:if>
            </tr>
        </c:forEach>
    </table>
